...
exports.insertNewPostWithImage = async (req, res, next) => {
  ...
  file.originalname = iconv.decode(
    Buffer.from(req.body.fileName, "latin1"),
    "utf8"
  );

  let date = new Date();
  let formData = req.body;
  OrignalFileName = req.body.fileName.split(".");
  NameLength = req.body.fileName.length;
  ...
  postOperation.GetPosts().then((result) => {
    for (let x = 0; x <= result.length - 1; x++) {
      ...
      personOperation
        ...
        .then((PersResult) => {
          try {
            if (PersResult[0].imgId > 0) {
              // console.log(PersResult[0].imgId);
              ImageOperation.getImageFromId(PersResult[0].imgId | 1).then(
                (Pimgresult) => {
                  PersResult[0].ProfileImage = Pimgresult[0];
                  result[x].personInfo = PersResult[0];
                  result[x].IsLike = 0;
                  if (Pimgresult[0].imgId == 5) console.log(Pimgresult);
                  personOperation
